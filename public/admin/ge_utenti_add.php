...
$layout->blocks["top"][] = "details";$page_layouts["ge_utenti_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "ge_utenti_inline_add.htm";
else
	$templatefile = "ge_utenti_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: ge_utenti_add.php');
...
//	processing id_tipologia - start
	...
	if($inlineAddOption)
	{
		...
		$control_id_tipologia->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing cognome - start
	...
	if($inlineAddOption)
	{
		...
		$control_cognome->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing data_registrazione - start
	...
	if($inlineAddOption)
	{
		...
		$control_data_registrazione->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing ip_registrazione - start
	...
	if($inlineAddOption)
	{
		...
		$control_ip_registrazione->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing numero_login - start
	...
	if($inlineAddOption)
	{
		...
		$control_numero_login->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing ip_ultimo_login - start
	...
	if($inlineAddOption)
	{
		...
		$control_ip_ultimo_login->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="vv_tipologia_utenti")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["id_tipologia"]==""){
			$avalues["id_tipologia"] = prepare_for_db("id_tipologia",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'ge_utenti_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'ge_utenti_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ge_utenti_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="vv_tipologia_utenti")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["id_tipologia"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["id_tipologia"]=@$avalues["id_tipologia"];
	...
	$defvalues["data_registrazione"]=@$avalues["data_registrazione"];
	$defvalues["ip_registrazione"]=@$avalues["ip_registrazione"];
	$defvalues["numero_login"]=@$avalues["numero_login"];
	$defvalues["ip_ultimo_login"]=@$avalues["ip_ultimo_login"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("id_tipologia_label","<label for=\"".GetInputElementId("id_tipologia", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("cognome_label","<label for=\"".GetInputElementId("cognome", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("data_registrazione_label","<label for=\"".GetInputElementId("data_registrazione", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ip_registrazione_label","<label for=\"".GetInputElementId("ip_registrazione", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("numero_login_label","<label for=\"".GetInputElementId("numero_login", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ip_ultimo_login_label","<label for=\"".GetInputElementId("ip_ultimo_login", $id, PAGE_ADD)."\">","</label>");
