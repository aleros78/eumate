...
include("include/pl_locations_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["pl_locations_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pl_locations_inline_edit.htm" : "pl_locations_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pl_locations_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pl_locations_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pl_locations";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pl_locations_view.php?".$viewlink."'\"");
