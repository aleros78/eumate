...
$layout->blocks["top"][] = "details";$page_layouts["pl_regioni_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pl_regioni_inline_add.htm";
else
	$templatefile = "pl_regioni_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pl_regioni_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing regione - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_regione = $pageObject->getControl("regione", $id);
		$control_regione->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pl_regioni_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pl_regioni_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pl_regioni_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["regione"]=@$avalues["regione"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("regione_label","<label for=\"".GetInputElementId("regione", $id, PAGE_ADD)."\">","</label>");
