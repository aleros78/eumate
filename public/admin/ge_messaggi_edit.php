...
$layout->blocks["top"][] = "details";$page_layouts["ge_messaggi_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "ge_messaggi_inline_edit.htm" : "ge_messaggi_edit.htm";
...
//	processing id_personaggio - begin
	$condition = 1;

	if($condition)
	{
		$control_id_personaggio = $pageObject->getControl("id_personaggio", $id);
		$control_id_personaggio->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: ge_messaggi_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: ge_messaggi_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["id_personaggio"] = $evalues["id_personaggio"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("id_personaggio_label","<label for=\"".GetInputElementId("id_personaggio", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='ge_messaggi_view.php?".$viewlink."'\"");
