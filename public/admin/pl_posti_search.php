...
$layout->blocks["top"][] = "search";$page_layouts["pl_posti_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pl_posti_search.htm";
...
$params['shortTableName'] = 'pl_posti';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("id_location"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("id_location")] = GetTableURL($pageObject->pSet->getLookupTable("id_location"));
	
	$pageObject->fillFieldToolTips("id_location");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("id_location");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "id_location";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("id_location_label","<label for=\"".GetInputElementId("id_location", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("id_location_label", true);
	
	$xt->assign("id_location_fieldblock", true);
	$xt->assignbyref("id_location_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("id_location_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("id_location_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_id_location", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("id_location");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"id_location", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"id_location", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("id_citta"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("id_citta")] = GetTableURL($pageObject->pSet->getLookupTable("id_citta"));
	
	$pageObject->fillFieldToolTips("id_citta");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("id_citta");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "id_citta";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("id_citta_label","<label for=\"".GetInputElementId("id_citta", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("id_citta_label", true);
	
	$xt->assign("id_citta_fieldblock", true);
	$xt->assignbyref("id_citta_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("id_citta_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("id_citta_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_id_citta", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("id_citta");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"id_citta", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"id_citta", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if($pageObject->pSet->getLookupTable("posto"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("posto")] = GetTableURL($pageObject->pSet->getLookupTable("posto"));
	
	$pageObject->fillFieldToolTips("posto");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("posto");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("posto_label","<label for=\"".GetInputElementId("posto", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("posto_label", true);
	
	$xt->assign("posto_fieldblock", true);
	$xt->assignbyref("posto_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("posto_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("posto_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_posto", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("posto");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"posto", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"posto", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
