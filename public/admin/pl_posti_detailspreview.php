...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["pl_posti_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= GetFullFieldName("id_citta", $strTableName, false)."=".make_db_value("id_citta",$_SESSION[$strTableName."_masterkey1"]);
}
if($mastertable == "pl_locations")
{
	$where = "";
		$where .= GetFullFieldName("id_location", $strTableName, false)."=".make_db_value("id_location",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	id_location - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("id_location", $data, $keylink);
			$row["id_location_value"] = $value;
	//	id_citta - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("id_citta", $data, $keylink);
			$row["id_citta_value"] = $value;
	//	id_regione - 
			$viewContainer->recId = $recordsCounter;
	...
	//	posto - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("posto", $data, $keylink);
			$row["posto_value"] = $value;
	//	nome_it - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template("pl_posti_detailspreview.htm");
