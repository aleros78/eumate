...
$layout->blocks["top"][] = "search";$page_layouts["pl_riferimenti_posti_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pl_riferimenti_posti_search.htm";
...
$params['shortTableName'] = 'pl_riferimenti_posti';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("id_posto_riferimento"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("id_posto_riferimento")] = GetTableURL($pageObject->pSet->getLookupTable("id_posto_riferimento"));
	
	$pageObject->fillFieldToolTips("id_posto_riferimento");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("id_posto_riferimento");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "id_posto_riferimento";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("id_posto_riferimento_label","<label for=\"".GetInputElementId("id_posto_riferimento", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("id_posto_riferimento_label", true);
	
	$xt->assign("id_posto_riferimento_fieldblock", true);
	$xt->assignbyref("id_posto_riferimento_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("id_posto_riferimento_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("id_posto_riferimento_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_id_posto_riferimento", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("id_posto_riferimento");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"id_posto_riferimento", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"id_posto_riferimento", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
