...
include_once("include/pl_riferimenti_posti_settings.php");
...
$layout = new TLayout("list2","BoldBlueWave","MobileBlueWave");
...
$layout->blocks["top"][] = "search";$page_layouts["pl_posti_list"] = $layout;
...
$layout->blocks["bare"][] = "mastergrid";$page_layouts["pl_citta_masterlist"] = $layout;

$layout = GetPageLayout("pl_citta", 'masterlist');
if($layout)
{
...
$viewControls = new ViewControlsContainer(new ProjectSettings("pl_citta", PAGE_LIST), PAGE_LIST);
...
include_once getabspath('classes/controls/ViewControlsContainer.php');
$viewControls = new ViewControlsContainer(new ProjectSettings("pl_locations", PAGE_LIST), PAGE_LIST);
$viewControls->addControlsJSAndCSS();
$pageObject->includes_js = array_merge($pageObject->includes_js, $viewControls->includes_js);
$pageObject->includes_jsreq = array_merge($pageObject->includes_jsreq, $viewControls->includes_jsreq);
$pageObject->includes_css = array_merge($pageObject->includes_css, $viewControls->includes_css);
...
$layout->skins["mastergrid"] = "grid";
$layout->blocks["bare"][] = "mastergrid";$page_layouts["pl_regioni_masterlist"] = $layout;

$layout = GetPageLayout("pl_regioni", 'masterlist');
if($layout)
{
	$rtl = $xt->getReadingOrder() == 'RTL' ? 'RTL' : '';
	$xt->cssFiles[] = array("stylepath" => "styles/".$layout->style.'/style'.$rtl.".css"
...
include_once getabspath('classes/controls/ViewControlsContainer.php');
$viewControls = new ViewControlsContainer(new ProjectSettings("pl_regioni", PAGE_LIST), PAGE_LIST);
$viewControls->addControlsJSAndCSS();
$pageObject->includes_js = array_merge($pageObject->includes_js, $viewControls->includes_js);
$pageObject->includes_jsreq = array_merge($pageObject->includes_jsreq, $viewControls->includes_jsreq);
$pageObject->includes_css = array_merge($pageObject->includes_css, $viewControls->includes_css);
