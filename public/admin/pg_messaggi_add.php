...
$layout->blocks["top"][] = "details";$page_layouts["pg_messaggi_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pg_messaggi_inline_add.htm";
else
	$templatefile = "pg_messaggi_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pg_messaggi_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pg_messaggi_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pg_messaggi_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pg_messaggi_".$pageObject->getPageType().".php");
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pg_messaggi";
		$options['firstTime'] = 1;
