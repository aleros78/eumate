...
$layout->blocks["top"][] = "details";$page_layouts["vv_config_var_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "vv_config_var_inline_add.htm";
else
	$templatefile = "vv_config_var_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: vv_config_var_add.php');
...
//	processing key - start
	...
	if($inlineAddOption)
	{
		...
		$control_key->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing key - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'vv_config_var_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'vv_config_var_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: vv_config_var_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["key"]=@$avalues["key"];
	$defvalues["value"]=@$avalues["value"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("key_label","<label for=\"".GetInputElementId("key", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("value_label","<label for=\"".GetInputElementId("value", $id, PAGE_ADD)."\">","</label>");
