...
$layout->blocks["top"][] = "details";$page_layouts["ge_testo_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "ge_testo_inline_add.htm";
else
	$templatefile = "ge_testo_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: ge_testo_add.php');
...
//	processing chiave - start
	...
	if($inlineAddOption)
	{
		...
		$control_chiave->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing value_it - start
	...
	if($inlineAddOption)
	{
		...
		$control_value_it->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing value_en - start
	...
	if($inlineAddOption)
	{
		...
		$control_value_en->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'ge_testo_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'ge_testo_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ge_testo_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["chiave"]=@$avalues["chiave"];
	$defvalues["value_it"]=@$avalues["value_it"];
	$defvalues["value_en"]=@$avalues["value_en"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("chiave_label","<label for=\"".GetInputElementId("chiave", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("value_it_label","<label for=\"".GetInputElementId("value_it", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("value_en_label","<label for=\"".GetInputElementId("value_en", $id, PAGE_ADD)."\">","</label>");
