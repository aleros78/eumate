...
add_nocache_headers();
include("include/pg_mail_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["pg_mail_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pg_mail_inline_add.htm";
else
	$templatefile = "pg_mail_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pg_mail_add.php');
...
//	processing invia - start
	...
	if($inlineAddOption)
	{
		...
		$control_invia->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing ricevi - start
	...
	if($inlineAddOption)
	{
		...
		$control_ricevi->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing messaggio - start
	...
	if($inlineAddOption)
	{
		...
		$control_messaggio->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing soggetto - start
	...
	if($inlineAddOption)
	{
		...
		$control_soggetto->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing data - start
	...
	if($inlineAddOption)
	{
		...
		$control_data->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing data - end
//	processing aperta - start
	...
	if($inlineAddOption)
	{
		...
		$control_aperta->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing vista_invia - start
	...
	if($inlineAddOption)
	{
		...
		$control_vista_invia->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing vista_ricevi - start
	...
	if($inlineAddOption)
	{
		...
		$control_vista_ricevi->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing vista_ricevi - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="pg_personaggi")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["invia"]==""){
			$avalues["invia"] = prepare_for_db("invia",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pg_mail_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pg_mail_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pg_mail_".$pageObject->getPageType().".php");
...
{
	$defvalues["aperta"] = 0;
	$defvalues["vista_invia"] = 1;
	$defvalues["vista_ricevi"] = 1;
}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="pg_personaggi")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["invia"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["invia"]=@$avalues["invia"];
	$defvalues["ricevi"]=@$avalues["ricevi"];
	$defvalues["messaggio"]=@$avalues["messaggio"];
	$defvalues["soggetto"]=@$avalues["soggetto"];
	$defvalues["data"]=@$avalues["data"];
	$defvalues["aperta"]=@$avalues["aperta"];
	$defvalues["vista_invia"]=@$avalues["vista_invia"];
	$defvalues["vista_ricevi"]=@$avalues["vista_ricevi"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("invia"))
		$xt->assign("invia_fieldblock",true);
	else
		$xt->assign("invia_tabfieldblock",true);
	$xt->assign("invia_label",true);
	if(isEnableSection508())
		$xt->assign_section("invia_label","<label for=\"".GetInputElementId("invia", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("ricevi"))
		$xt->assign("ricevi_fieldblock",true);
	else
		$xt->assign("ricevi_tabfieldblock",true);
	$xt->assign("ricevi_label",true);
	if(isEnableSection508())
		$xt->assign_section("ricevi_label","<label for=\"".GetInputElementId("ricevi", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("messaggio"))
		$xt->assign("messaggio_fieldblock",true);
	else
		$xt->assign("messaggio_tabfieldblock",true);
	$xt->assign("messaggio_label",true);
	if(isEnableSection508())
		$xt->assign_section("messaggio_label","<label for=\"".GetInputElementId("messaggio", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("soggetto"))
		$xt->assign("soggetto_fieldblock",true);
	else
		$xt->assign("soggetto_tabfieldblock",true);
	$xt->assign("soggetto_label",true);
	if(isEnableSection508())
		$xt->assign_section("soggetto_label","<label for=\"".GetInputElementId("soggetto", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("data_label","<label for=\"".GetInputElementId("data", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("aperta"))
		$xt->assign("aperta_fieldblock",true);
	else
		$xt->assign("aperta_tabfieldblock",true);
	$xt->assign("aperta_label",true);
	if(isEnableSection508())
		$xt->assign_section("aperta_label","<label for=\"".GetInputElementId("aperta", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("vista_invia"))
		$xt->assign("vista_invia_fieldblock",true);
	else
		$xt->assign("vista_invia_tabfieldblock",true);
	$xt->assign("vista_invia_label",true);
	if(isEnableSection508())
		$xt->assign_section("vista_invia_label","<label for=\"".GetInputElementId("vista_invia", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("vista_ricevi"))
		$xt->assign("vista_ricevi_fieldblock",true);
	else
		$xt->assign("vista_ricevi_tabfieldblock",true);
	$xt->assign("vista_ricevi_label",true);
	if(isEnableSection508())
		$xt->assign_section("vista_ricevi_label","<label for=\"".GetInputElementId("vista_ricevi", $id, PAGE_ADD)."\">","</label>");
