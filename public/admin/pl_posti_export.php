...
$layout->blocks["top"][] = "export";$page_layouts["pl_posti_export"] = $layout;
...
$xt->display("pl_posti_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=pl_posti.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pl_posti.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pl_posti.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["id_location"] = $pageObject->showDBValue("id_location", $row);
			$values["id_citta"] = $pageObject->showDBValue("id_citta", $row);
			$values["id_regione"] = $pageObject->showDBValue("id_regione", $row);
			$values["posto"] = $pageObject->showDBValue("posto", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pl_posti.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["id_location"] = $pageObject->getViewControl("id_location")->showDBValue($row, "");
			$values["id_citta"] = $pageObject->getViewControl("id_citta")->showDBValue($row, "");
			$values["id_regione"] = $pageObject->getViewControl("id_regione")->showDBValue($row, "");
			$values["posto"] = $pageObject->getViewControl("posto")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["id_location"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["id_citta"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["id_regione"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["posto"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Id Location").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Id Citta").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Id Regione").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Posto").'</td>';	
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["id_location"]);
				else
					echo $values["id_location"];//echo htmlspecialchars($values["id_location"]); commented for bug #6823
			...
			echo '</td>';
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["id_citta"]);
				else
					echo $values["id_citta"];//echo htmlspecialchars($values["id_citta"]); commented for bug #6823
			...
			echo '</td>';
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["id_regione"]);
				else
					echo $values["id_regione"];//echo htmlspecialchars($values["id_regione"]); commented for bug #6823
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["posto"]);
					else
						echo $values["posto"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
