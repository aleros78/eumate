...
$layout->blocks["top"][] = "details";$page_layouts["pl_citta_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pl_citta_inline_edit.htm" : "pl_citta_edit.htm";
...
//	processing citta - begin
	$condition = 1;

	if($condition)
	{
		$control_citta = $pageObject->getControl("citta", $id);
		$control_citta->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pl_citta_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("citta_label","<label for=\"".GetInputElementId("citta", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pl_citta_view.php?".$viewlink."'\"");
