...
$layout->blocks["top"][] = "search";$page_layouts["pl_regioni_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pl_regioni_search.htm";
...
$params['shortTableName'] = 'pl_regioni';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("regione"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("regione")] = GetTableURL($pageObject->pSet->getLookupTable("regione"));
	
	$pageObject->fillFieldToolTips("regione");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("regione");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "regione";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("regione_label","<label for=\"".GetInputElementId("regione", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("regione_label", true);
	
	$xt->assign("regione_fieldblock", true);
	$xt->assignbyref("regione_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("regione_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("regione_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_regione", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("regione");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"regione", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"regione", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
