...
$layout->blocks["top"][] = "details";$page_layouts["ge_messaggi_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "ge_messaggi_inline_add.htm";
else
	$templatefile = "ge_messaggi_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: ge_messaggi_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing id_personaggio - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_id_personaggio = $pageObject->getControl("id_personaggio", $id);
		$control_id_personaggio->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing id_personaggio - end
...
//	insert masterkey value if exists and if not specified
	...
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="pg_personaggi")
	{
		...
		if($avalues["id_personaggio"]==""){
			$avalues["id_personaggio"] = prepare_for_db("id_personaggio",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing id_personaggio - start
			...
			if($inlineAddOption)
			{
				$control_id_personaggio->afterSuccessfulSave();
			}
//	processing id_personaggio - end
//	processing id_gilda - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'ge_messaggi_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'ge_messaggi_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ge_messaggi_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="pg_personaggi")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["id_personaggio"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	$defvalues["id_personaggio"]=@$avalues["id_personaggio"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("id_personaggio"))
		$xt->assign("id_personaggio_fieldblock",true);
	else
		$xt->assign("id_personaggio_tabfieldblock",true);
	$xt->assign("id_personaggio_label",true);
	if(isEnableSection508())
		$xt->assign_section("id_personaggio_label","<label for=\"".GetInputElementId("id_personaggio", $id, PAGE_ADD)."\">","</label>");
...
//	id_personaggio
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("id_personaggio", $data, $keylink);
		$showValues["id_personaggio"] = $value;
		$showFields[] = "id_personaggio";
	}	
//	id_gilda
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	data
	...
	{	
		...
		$showRawValues["id_personaggio"] = substr($data["id_personaggio"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "ge_messaggi";
		$options['firstTime'] = 1;
