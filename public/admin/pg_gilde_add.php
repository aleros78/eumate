...
$layout->blocks["top"][] = "details";$page_layouts["pg_gilde_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pg_gilde_inline_add.htm";
else
	$templatefile = "pg_gilde_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pg_gilde_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pg_gilde_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pg_gilde_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pg_gilde_".$pageObject->getPageType().".php");
