...
include("include/vv_config_var_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["vv_config_var_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "vv_config_var_inline_edit.htm" : "vv_config_var_edit.htm";
...
//	processing key - begin
	$condition = 1;

	if($condition)
	{
		$control_key = $pageObject->getControl("key", $id);
		$control_key->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
	...
	//	do event
		...
		{		
			...
			//	processing key - begin
				...
				if($condition)
				{
					$control_key->afterSuccessfulSave();
				}
	//	processing key - end
			//	processing value - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: vv_config_var_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: vv_config_var_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("key"))
		$xt->assign("key_fieldblock",true);
	else
		$xt->assign("key_tabfieldblock",true);
	$xt->assign("key_label",true);
	if(isEnableSection508())
		$xt->assign_section("key_label","<label for=\"".GetInputElementId("key", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("value"))
		$xt->assign("value_fieldblock",true);
	else
		$xt->assign("value_tabfieldblock",true);
	$xt->assign("value_label",true);
	if(isEnableSection508())
		$xt->assign_section("value_label","<label for=\"".GetInputElementId("value", $id, PAGE_EDIT)."\">","</label>");
...
//	key - 
	$value = $pageObject->showDBValue("key", $data, $keylink);
	$showValues["key"] = $value;
	$showFields[] = "key";
		$showRawValues["key"] = substr($data["key"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "vv_config_var";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='vv_config_var_view.php?".$viewlink."'\"");
