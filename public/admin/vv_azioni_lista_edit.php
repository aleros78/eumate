...
$layout->blocks["top"][] = "details";$page_layouts["vv_azioni_lista_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "vv_azioni_lista_inline_edit.htm" : "vv_azioni_lista_edit.htm";
...
//	processing azione - begin
	$condition = 1;

	if($condition)
	{
		$control_azione = $pageObject->getControl("azione", $id);
		$control_azione->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing azione - end
//	processing descrizione - begin
	$condition = 1;

	if($condition)
	{
		$control_descrizione = $pageObject->getControl("descrizione", $id);
		$control_descrizione->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
	...
	//	do event
		...
		{		
			...
			//	processing azione - begin
				...
				if($condition)
				{
					$control_azione->afterSuccessfulSave();
				}
	//	processing azione - end
			//	processing descrizione - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: vv_azioni_lista_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: vv_azioni_lista_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["azione"] = $evalues["azione"];
	$data["descrizione"] = $evalues["descrizione"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("azione"))
		$xt->assign("azione_fieldblock",true);
	else
		$xt->assign("azione_tabfieldblock",true);
	$xt->assign("azione_label",true);
	if(isEnableSection508())
		$xt->assign_section("azione_label","<label for=\"".GetInputElementId("azione", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("descrizione"))
		$xt->assign("descrizione_fieldblock",true);
	else
		$xt->assign("descrizione_tabfieldblock",true);
	$xt->assign("descrizione_label",true);
	if(isEnableSection508())
		$xt->assign_section("descrizione_label","<label for=\"".GetInputElementId("descrizione", $id, PAGE_EDIT)."\">","</label>");
...
//	azione - 
	$value = $pageObject->showDBValue("azione", $data, $keylink);
	$showValues["azione"] = $value;
	$showFields[] = "azione";
		$showRawValues["azione"] = substr($data["azione"],0,100);

//	descrizione - 
	$value = $pageObject->showDBValue("descrizione", $data, $keylink);
	$showValues["descrizione"] = $value;
	$showFields[] = "descrizione";
		$showRawValues["descrizione"] = substr($data["descrizione"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "vv_azioni_lista";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='vv_azioni_lista_view.php?".$viewlink."'\"");
