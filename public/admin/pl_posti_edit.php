...
$layout->blocks["top"][] = "details";$page_layouts["pl_posti_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pl_posti_inline_edit.htm" : "pl_posti_edit.htm";
...
//	processing id_location - begin
	$condition = 1;

	if($condition)
	{
		$control_id_location = $pageObject->getControl("id_location", $id);
		$control_id_location->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing id_location - end
//	processing id_citta - begin
	$condition = 1;

	if($condition)
	{
		$control_id_citta = $pageObject->getControl("id_citta", $id);
		$control_id_citta->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing id_citta - end
//	processing id_regione - begin
	$condition = 1;

	if($condition)
	{
		$control_id_regione = $pageObject->getControl("id_regione", $id);
		$control_id_regione->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing id_regione - end
//	processing posto - begin
	$condition = 1;

	if($condition)
	{
		$control_posto = $pageObject->getControl("posto", $id);
		$control_posto->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing posto - end
//	processing nome_it - begin
	$condition = 1;

	if($condition)
	{
		$control_nome_it = $pageObject->getControl("nome_it", $id);
		$control_nome_it->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing nome_it - end
//	processing nome_en - begin
	$condition = 1;

	if($condition)
	{
		$control_nome_en = $pageObject->getControl("nome_en", $id);
		$control_nome_en->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing nome_en - end
//	processing descrizione_it - begin
	$condition = 1;

	if($condition)
	{
		$control_descrizione_it = $pageObject->getControl("descrizione_it", $id);
		$control_descrizione_it->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing descrizione_it - end
//	processing descrizione_en - begin
	$condition = 1;

	if($condition)
	{
		$control_descrizione_en = $pageObject->getControl("descrizione_en", $id);
		$control_descrizione_en->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing descrizione_en - end
//	processing storia_it - begin
	$condition = 1;

	if($condition)
	{
		$control_storia_it = $pageObject->getControl("storia_it", $id);
		$control_storia_it->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing storia_it - end
//	processing storia_en - begin
	$condition = 1;

	if($condition)
	{
		$control_storia_en = $pageObject->getControl("storia_en", $id);
		$control_storia_en->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing storia_en - end
//	processing back_it - begin
	$condition = 1;

	if($condition)
	{
		$control_back_it = $pageObject->getControl("back_it", $id);
		$control_back_it->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing back_it - end
//	processing back_en - begin
	$condition = 1;

	if($condition)
	{
		$control_back_en = $pageObject->getControl("back_en", $id);
		$control_back_en->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing back_en - end
//	processing note_it - begin
	$condition = 1;

	if($condition)
	{
		$control_note_it = $pageObject->getControl("note_it", $id);
		$control_note_it->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing note_it - end
//	processing note_en - begin
	$condition = 1;

	if($condition)
	{
		$control_note_en = $pageObject->getControl("note_en", $id);
		$control_note_en->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing attivo - begin
	...
	if($condition)
	{
		...
		$control_attivo->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing attivo - end
	...
	//	do event
		...
		{		
			...
			//	processing id_location - begin
				...
				if($condition)
				{
					$control_id_location->afterSuccessfulSave();
				}
	//	processing id_location - end
			//	processing id_citta - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_id_citta->afterSuccessfulSave();
				}
	//	processing id_citta - end
			//	processing id_regione - begin
							$condition = 1;
	...
	//	processing id_regione - end
			//	processing posto - begin
				...
				if($condition)
				{
					$control_posto->afterSuccessfulSave();
				}
	//	processing posto - end
			//	processing nome_it - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pl_posti_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pl_posti_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["id_location"] = $evalues["id_location"];
	$data["id_citta"] = $evalues["id_citta"];
	$data["id_regione"] = $evalues["id_regione"];
	$data["posto"] = $evalues["posto"];
	$data["nome_it"] = $evalues["nome_it"];
	$data["nome_en"] = $evalues["nome_en"];
	$data["descrizione_it"] = $evalues["descrizione_it"];
	$data["descrizione_en"] = $evalues["descrizione_en"];
	$data["storia_it"] = $evalues["storia_it"];
	$data["storia_en"] = $evalues["storia_en"];
	$data["back_it"] = $evalues["back_it"];
	$data["back_en"] = $evalues["back_en"];
	$data["note_it"] = $evalues["note_it"];
	$data["note_en"] = $evalues["note_en"];
	$data["attivo"] = $evalues["attivo"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("id_location"))
		$xt->assign("id_location_fieldblock",true);
	else
		$xt->assign("id_location_tabfieldblock",true);
	$xt->assign("id_location_label",true);
	if(isEnableSection508())
		$xt->assign_section("id_location_label","<label for=\"".GetInputElementId("id_location", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("id_citta"))
		$xt->assign("id_citta_fieldblock",true);
	else
		$xt->assign("id_citta_tabfieldblock",true);
	$xt->assign("id_citta_label",true);
	if(isEnableSection508())
		$xt->assign_section("id_citta_label","<label for=\"".GetInputElementId("id_citta", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("id_regione_label","<label for=\"".GetInputElementId("id_regione", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("posto"))
		$xt->assign("posto_fieldblock",true);
	else
		$xt->assign("posto_tabfieldblock",true);
	$xt->assign("posto_label",true);
	if(isEnableSection508())
		$xt->assign_section("posto_label","<label for=\"".GetInputElementId("posto", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("nome_it_label","<label for=\"".GetInputElementId("nome_it", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("nome_en_label","<label for=\"".GetInputElementId("nome_en", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("descrizione_it_label","<label for=\"".GetInputElementId("descrizione_it", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("descrizione_en_label","<label for=\"".GetInputElementId("descrizione_en", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("storia_it_label","<label for=\"".GetInputElementId("storia_it", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("storia_en_label","<label for=\"".GetInputElementId("storia_en", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("back_it_label","<label for=\"".GetInputElementId("back_it", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("back_en_label","<label for=\"".GetInputElementId("back_en", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("note_it_label","<label for=\"".GetInputElementId("note_it", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("note_en_label","<label for=\"".GetInputElementId("note_en", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("attivo_label","<label for=\"".GetInputElementId("attivo", $id, PAGE_EDIT)."\">","</label>");
...
//	id_location - 
	$value = $pageObject->showDBValue("id_location", $data, $keylink);
	$showValues["id_location"] = $value;
	$showFields[] = "id_location";
		$showRawValues["id_location"] = substr($data["id_location"],0,100);

//	id_citta - 
	$value = $pageObject->showDBValue("id_citta", $data, $keylink);
	$showValues["id_citta"] = $value;
	$showFields[] = "id_citta";
		$showRawValues["id_citta"] = substr($data["id_citta"],0,100);
...
//	posto - 
	$value = $pageObject->showDBValue("posto", $data, $keylink);
	$showValues["posto"] = $value;
	$showFields[] = "posto";
		$showRawValues["posto"] = substr($data["posto"],0,100);
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pl_posti_view.php?".$viewlink."'\"");
