...
$layout->blocks["top"][] = "details";$page_layouts["pl_posti_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pl_posti_inline_add.htm";
else
	$templatefile = "pl_posti_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pl_posti_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing id_location - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_id_location = $pageObject->getControl("id_location", $id);
		$control_id_location->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing id_location - end
//	processing id_citta - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_id_citta = $pageObject->getControl("id_citta", $id);
		$control_id_citta->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing posto - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_posto = $pageObject->getControl("posto", $id);
		$control_posto->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="pl_citta")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["id_citta"]==""){
			$avalues["id_citta"] = prepare_for_db("id_citta",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			
	}
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="pl_locations")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["id_location"]==""){
			$avalues["id_location"] = prepare_for_db("id_location",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pl_posti_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pl_posti_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pl_posti_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="pl_citta")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["id_citta"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="pl_locations")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["id_location"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	$defvalues["id_location"]=@$avalues["id_location"];
	$defvalues["id_citta"]=@$avalues["id_citta"];
	$defvalues["id_regione"]=@$avalues["id_regione"];
	$defvalues["posto"]=@$avalues["posto"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("id_location_label","<label for=\"".GetInputElementId("id_location", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("id_citta_label","<label for=\"".GetInputElementId("id_citta", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("posto_label","<label for=\"".GetInputElementId("posto", $id, PAGE_ADD)."\">","</label>");
