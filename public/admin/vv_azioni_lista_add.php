...
$layout->blocks["top"][] = "details";$page_layouts["vv_azioni_lista_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "vv_azioni_lista_inline_add.htm";
else
	$templatefile = "vv_azioni_lista_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: vv_azioni_lista_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing azione - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_azione = $pageObject->getControl("azione", $id);
		$control_azione->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing descrizione - start
	...
	if($inlineAddOption)
	{
		...
		$control_descrizione->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'vv_azioni_lista_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'vv_azioni_lista_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: vv_azioni_lista_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["azione"]=@$avalues["azione"];
	$defvalues["descrizione"]=@$avalues["descrizione"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("azione_label","<label for=\"".GetInputElementId("azione", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("descrizione_label","<label for=\"".GetInputElementId("descrizione", $id, PAGE_ADD)."\">","</label>");
