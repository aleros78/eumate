...
function checkTableName($shortTName, $type=false)
{
	...
	if ("ge_utenti" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("pg_mail" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("pg_gilde" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("pl_locations" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("pg_personaggi" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("pl_citta" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("pl_posti" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("pl_regioni" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("pl_riferimenti_posti" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("vv_azioni_lista" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
...
function GetUserPermissions($table="")
{
	...
	if($table=="pl_locations")
	{
			$permissions =  "ADESPIM";
	}
	if($table=="pg_personaggi")
	{
			$permissions =  "ADESPIM";
	}
	if($table=="pl_citta")
	...
	if($table=="pl_riferimenti_posti")
	{
			$permissions =  "ADESPIM";
	}
	if($table=="vv_azioni_lista")
	{
			$permissions =  "ADESPIM";
	}
	if($table=="vv_config_var")
	{
			$permissions =  "ADESPIM";
	}
