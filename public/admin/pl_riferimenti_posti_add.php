...
add_nocache_headers();
include("include/pl_riferimenti_posti_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["pl_riferimenti_posti_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pl_riferimenti_posti_inline_add.htm";
else
	$templatefile = "pl_riferimenti_posti_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pl_riferimenti_posti_add.php');
...
//	processing id_posto_riferimento - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_id_posto_riferimento = $pageObject->getControl("id_posto_riferimento", $id);
		$control_id_posto_riferimento->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing id_posto_riferimento - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="pl_posti")
	{
		...
		if($avalues["id_posto_riferimento"]==""){
			$avalues["id_posto_riferimento"] = prepare_for_db("id_posto_riferimento",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing id_posto_riferimento - start
			...
			if($inlineAddOption)
			{
				$control_id_posto_riferimento->afterSuccessfulSave();
			}
//	processing id_posto_riferimento - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pl_riferimenti_posti_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pl_riferimenti_posti_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pl_riferimenti_posti_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="pl_posti")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["id_posto_riferimento"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	...
	$defvalues["id_posto_riferimento"]=@$avalues["id_posto_riferimento"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("id_posto_riferimento"))
		$xt->assign("id_posto_riferimento_fieldblock",true);
	else
		$xt->assign("id_posto_riferimento_tabfieldblock",true);
	$xt->assign("id_posto_riferimento_label",true);
	if(isEnableSection508())
		$xt->assign_section("id_posto_riferimento_label","<label for=\"".GetInputElementId("id_posto_riferimento", $id, PAGE_ADD)."\">","</label>");
...
//	id_posto_riferimento
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("id_posto_riferimento", $data, $keylink);
		$showValues["id_posto_riferimento"] = $value;
		$showFields[] = "id_posto_riferimento";
	}	
		$showRawValues["id"] = substr($data["id"],0,100);
		$showRawValues["id_posto"] = substr($data["id_posto"],0,100);
		$showRawValues["id_posto_riferimento"] = substr($data["id_posto_riferimento"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pl_riferimenti_posti";
		$options['firstTime'] = 1;
