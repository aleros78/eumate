...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["pl_riferimenti_posti_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= GetFullFieldName("id_posto_riferimento", $strTableName, false)."=".make_db_value("id_posto_riferimento",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	id_posto_riferimento - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("id_posto_riferimento", $data, $keylink);
			$row["id_posto_riferimento_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("pl_riferimenti_posti_detailspreview.htm");
