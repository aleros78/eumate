...
include("include/pg_messaggi_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["pg_messaggi_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pg_messaggi_search.htm";
...
$params['shortTableName'] = 'pg_messaggi';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("id_personaggio"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("id_personaggio")] = GetTableURL($pageObject->pSet->getLookupTable("id_personaggio"));
	
	$pageObject->fillFieldToolTips("id_personaggio");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("id_personaggio");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "id_personaggio";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("id_personaggio_label","<label for=\"".GetInputElementId("id_personaggio", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("id_personaggio_label", true);
	
	$xt->assign("id_personaggio_fieldblock", true);
	$xt->assignbyref("id_personaggio_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("id_personaggio_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("id_personaggio_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_id_personaggio", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("id_personaggio");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"id_personaggio", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"id_personaggio", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if($pageObject->pSet->getLookupTable("messaggio"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("messaggio")] = GetTableURL($pageObject->pSet->getLookupTable("messaggio"));
	
	$pageObject->fillFieldToolTips("messaggio");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("messaggio");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "messaggio";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("messaggio_label","<label for=\"".GetInputElementId("messaggio", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("messaggio_label", true);
	
	$xt->assign("messaggio_fieldblock", true);
	$xt->assignbyref("messaggio_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("messaggio_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("messaggio_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_messaggio", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("messaggio");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"messaggio", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"messaggio", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("data"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("data")] = GetTableURL($pageObject->pSet->getLookupTable("data"));
	...
	if(isEnableSection508())
		$xt->assign_section("data_label","<label for=\"".GetInputElementId("data", $id, PAGE_SEARCH)."\">","</label>");
	else 
