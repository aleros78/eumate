...
$layout->blocks["top"][] = "export";$page_layouts["vv_azioni_lista_export"] = $layout;
...
$xt->display("vv_azioni_lista_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=vv_azioni_lista.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=vv_azioni_lista.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=vv_azioni_lista.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["azione"] = $pageObject->showDBValue("azione", $row);
			$values["descrizione"] = $pageObject->showDBValue("descrizione", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=vv_azioni_lista.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["azione"] = $pageObject->getViewControl("azione")->showDBValue($row, "");
			$values["descrizione"] = $pageObject->getViewControl("descrizione")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["azione"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["descrizione"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Azione").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Descrizione").'</td>';	
	}
	else
	{
		echo "<td>"."Id"."</td>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["azione"]);
					else
						echo $values["azione"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["descrizione"]);
					else
						echo $values["descrizione"];
			echo '</td>';
			echo "</tr>";
		}
