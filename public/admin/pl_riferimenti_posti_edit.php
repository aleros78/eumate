...
$layout->blocks["top"][] = "details";$page_layouts["pl_riferimenti_posti_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pl_riferimenti_posti_inline_edit.htm" : "pl_riferimenti_posti_edit.htm";
...
//	processing id_posto_riferimento - begin
	$condition = 1;

	if($condition)
	{
		$control_id_posto_riferimento = $pageObject->getControl("id_posto_riferimento", $id);
		$control_id_posto_riferimento->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pl_riferimenti_posti_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pl_riferimenti_posti_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["id_posto_riferimento"] = $evalues["id_posto_riferimento"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("id_posto_riferimento_label","<label for=\"".GetInputElementId("id_posto_riferimento", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pl_riferimenti_posti_view.php?".$viewlink."'\"");
