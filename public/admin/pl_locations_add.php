...
$layout->blocks["top"][] = "details";$page_layouts["pl_locations_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pl_locations_inline_add.htm";
else
	$templatefile = "pl_locations_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pl_locations_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pl_locations_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pl_locations_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pl_locations_".$pageObject->getPageType().".php");
