...
$layout->blocks["top"][] = "details";$page_layouts["vv_tipologia_utenti_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "vv_tipologia_utenti_inline_add.htm";
else
	$templatefile = "vv_tipologia_utenti_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: vv_tipologia_utenti_add.php');
...
//	processing tipologia - start
	...
	if($inlineAddOption)
	{
		...
		$control_tipologia->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'vv_tipologia_utenti_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'vv_tipologia_utenti_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: vv_tipologia_utenti_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["tipologia"]=@$avalues["tipologia"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("tipologia_label","<label for=\"".GetInputElementId("tipologia", $id, PAGE_ADD)."\">","</label>");
