...
$layout->blocks["top"][] = "details";$page_layouts["vv_tipologia_utenti_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "vv_tipologia_utenti_inline_edit.htm" : "vv_tipologia_utenti_edit.htm";
...
//	processing tipologia - begin
	$condition = 1;

	if($condition)
	{
		$control_tipologia = $pageObject->getControl("tipologia", $id);
		$control_tipologia->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing tipologia - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: vv_tipologia_utenti_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: vv_tipologia_utenti_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["tipologia"] = $evalues["tipologia"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("tipologia"))
		$xt->assign("tipologia_fieldblock",true);
	else
		$xt->assign("tipologia_tabfieldblock",true);
	$xt->assign("tipologia_label",true);
	if(isEnableSection508())
		$xt->assign_section("tipologia_label","<label for=\"".GetInputElementId("tipologia", $id, PAGE_EDIT)."\">","</label>");
...
//	detail tables
	$showDetailKeys["ge_utenti"]["masterkey1"] = $data["id"];		
...
//	tipologia - 
	$value = $pageObject->showDBValue("tipologia", $data, $keylink);
	$showValues["tipologia"] = $value;
	$showFields[] = "tipologia";
		$showRawValues["tipologia"] = substr($data["tipologia"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "vv_tipologia_utenti";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='vv_tipologia_utenti_view.php?".$viewlink."'\"");
