...
$layout->blocks["top"][] = "details";$page_layouts["pl_regioni_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pl_regioni_inline_edit.htm" : "pl_regioni_edit.htm";
...
//	processing regione - begin
	$condition = 1;

	if($condition)
	{
		$control_regione = $pageObject->getControl("regione", $id);
		$control_regione->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing regione - end
	...
	//	do event
		...
		{		
			...
			//	processing regione - begin
				...
				if($condition)
				{
					$control_regione->afterSuccessfulSave();
				}
	//	processing regione - end
			//	processing nome_it - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pl_regioni_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pl_regioni_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("regione"))
		$xt->assign("regione_fieldblock",true);
	else
		$xt->assign("regione_tabfieldblock",true);
	$xt->assign("regione_label",true);
	if(isEnableSection508())
		$xt->assign_section("regione_label","<label for=\"".GetInputElementId("regione", $id, PAGE_EDIT)."\">","</label>");
...
//	detail tables
	$showDetailKeys["pl_citta"]["masterkey1"] = $data["id"];		
...
//	regione - 
	$value = $pageObject->showDBValue("regione", $data, $keylink);
	$showValues["regione"] = $value;
	$showFields[] = "regione";
		$showRawValues["regione"] = substr($data["regione"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pl_regioni";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pl_regioni_view.php?".$viewlink."'\"");
