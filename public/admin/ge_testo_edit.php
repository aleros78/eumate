...
$layout->blocks["top"][] = "details";$page_layouts["ge_testo_edit"] = $layout;
...
//	processing chiave - begin
	...
	if($condition)
	{
		...
		$control_chiave->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing value_it - begin
	...
	if($condition)
	{
		...
		$control_value_it->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing value_en - begin
	...
	if($condition)
	{
		...
		$control_value_en->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing value_en - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: ge_testo_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: ge_testo_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["chiave"] = $evalues["chiave"];
	$data["value_it"] = $evalues["value_it"];
	$data["value_en"] = $evalues["value_en"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("chiave"))
		$xt->assign("chiave_fieldblock",true);
	else
		$xt->assign("chiave_tabfieldblock",true);
	$xt->assign("chiave_label",true);
	if(isEnableSection508())
		$xt->assign_section("chiave_label","<label for=\"".GetInputElementId("chiave", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("value_it"))
		$xt->assign("value_it_fieldblock",true);
	else
		$xt->assign("value_it_tabfieldblock",true);
	$xt->assign("value_it_label",true);
	if(isEnableSection508())
		$xt->assign_section("value_it_label","<label for=\"".GetInputElementId("value_it", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("value_en"))
		$xt->assign("value_en_fieldblock",true);
	else
		$xt->assign("value_en_tabfieldblock",true);
	$xt->assign("value_en_label",true);
	if(isEnableSection508())
		$xt->assign_section("value_en_label","<label for=\"".GetInputElementId("value_en", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='ge_testo_view.php?".$viewlink."'\"");
